...
public class ObjectManipulator : MonoBehaviour
{
    ...
    IEnumerator DownloadImage(string MediaUrl)
    {
        ...
        {
            m_Renderer = toTrace.GetComponent<Renderer>();// = ;
            m_Renderer.material.SetTexture("_MainTex", ((DownloadHandlerTexture)request.downloadHandler).texture);
        }
    }
    ...
    private void Start()
    {
        ...
        ui_text.text += UrlSuffix + BaseURL();
    ...
    void Update()
    {
        ...
        Vector3 controllerPosL = OVRInput.GetLocalControllerPosition(l_controller);
        Quaternion controllerRotL = OVRInput.GetLocalControllerRotation(l_controller);

        FindHoverObject(controllerPos, controllerRot);

        if (OVRInput.GetDown(OVRInput.Button.Two, r_controller))
        { // top button
            passthrough.textureOpacity -= .1f;


        }
        else if (OVRInput.GetDown(OVRInput.Button.One, r_controller))
        { // bottom button
            passthrough.textureOpacity += .1f;
        }

        if (OVRInput.GetDown(OVRInput.Button.PrimaryThumbstick, r_controller))
        {
            RetrieveBtn();
            passthrough.textureOpacity += .2f;
        }


        if (hoverObject)
        {
            if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, r_controller))
            {
                // grabbing
        ...
        if (grabObject)
        {
            ...
            ManipulateObjectL(grabObject, controllerPosL, controllerRotL);

            if (OVRInput.GetDown(OVRInput.Button.One, l_controller))
            {
                Vector3 objectScale = grabObject.transform.localScale;
                grabObject.transform.localScale = new Vector3(objectScale.x * .95f, objectScale.y * .95f, objectScale.z * .95f);
            }
            else if (OVRInput.GetDown(OVRInput.Button.Two, l_controller))
            {
                Vector3 objectScale = grabObject.transform.localScale;
                grabObject.transform.localScale = new Vector3(objectScale.x * 1.05f, objectScale.y * 1.05f, objectScale.z * 1.05f);
            }
            else if (OVRInput.GetDown(OVRInput.Button.PrimaryHandTrigger, l_controller))
            {
                Vector3 objectScale = grabObject.transform.localScale;
                grabObject.transform.localScale = new Vector3(objectScale.x * 1.05f, objectScale.y, objectScale.z);
            }
            else if (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger, l_controller))
            {
                Vector3 objectScale = grabObject.transform.localScale;
                grabObject.transform.localScale = new Vector3(objectScale.x * .95f, objectScale.y, objectScale.z);
            }
    ...
    void ManipulateObjectL(GameObject obj, Vector3 controllerPos, Quaternion controllerRot)
    {
        Vector2 thumbstick = OVRInput.Get(OVRInput.Axis2D.PrimaryThumbstick, l_controller);

        obj.transform.Rotate(Vector3.left * -thumbstick.y);
    ...
    void CheckForDominantHand()
    {
        ...
        if (r_controller == OVRInput.Controller.RTouch)
        {
            if (OVRInput.Get(OVRInput.RawButton.LHandTrigger))
            {
                r_controller = OVRInput.Controller.LTouch;
            }
        }
        else
        {
